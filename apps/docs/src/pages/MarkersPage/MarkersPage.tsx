...
const MarkersPage: FC = () => {
  return (
    <Box>
      ...
      <DemoSection
        title="GPU text labels — SymbolLayer"
        description={
          <>
            When you need hundreds of labels rather than rich interactive
            content, <code>&lt;SymbolLayer&gt;</code> renders them as a single
            GPU symbol layer — MapLibre declutters collisions automatically as
            you zoom, and an optional icon can sit beneath each label. Text and
            halo colors accept palette tokens.
          </>
        }
        code={symbolLabelsSource}
        demo={<SymbolLabels />}
      />
      <PropsTable component="Marker" />
      <PropsTable component="SymbolLayer" />
    </Box>
  );
};
